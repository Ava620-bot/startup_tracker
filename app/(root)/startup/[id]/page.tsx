...
export const experimental_ppr = true; //ppr is enabled for this page
...
export default async function StartupPage({ params }: { params: { id: string } }) {
  ...
  try {
    // parallel rendering - data fetching
    ...
    return (
      <>
        ...
        <section className='section_container'>
          <img src={post?.image} alt="thumbnail" className='w-full h-[50%] rounded-xl' />
          <div className="space-y-5 mt-10 max-w-4xl mx-auto">
            <div className="flex-between gap-5">
              <Link href={`/user/${post?.author?._id}`} className='flex gap-2 items-center mb-3'>
                <Image src={post?.author?.image} alt='author-image' className='rounded-full drop-shadow-lg object-contain' width={64} height={64} />
            ...
            <div className="max-w-4xl mx-auto">
              ...
              <ul className='mt-7 card_grid-sm'>
                {editorPosts.map((post: StartupTypeCard, index: number) => (
                 <StartupCards post={post} key={index} />
                ))}
              </ul>
            </div>
          )}
